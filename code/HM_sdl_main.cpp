...
void HM_RenderTexture() {
  if (SDL_UpdateTexture(hm_sdl.bitmapTexture, 0, hm_sdl.bitmapMemory,
                        hm_sdl.pitch)) {
...
void HM_SDLSetupTexture() {
  ...
  hm_sdl.bitmapTexture =
      SDL_CreateTexture(hm_sdl.renderer, SDL_PIXELFORMAT_ARGB8888,
...
void HM_SdlLoadControls(SDL_GameController *ctrler) {
  for (int ctrlerIdx = 0; ctrlerIdx < maxControllers; ++ctrlerIdx) {
    ...
    ctrler.RightShoulder = SDL_GameControllerGetButton(
    ...
    SDL_LogDebug(0, "Started controller index %d named: %s", ctrlerIdx,
                 ctrlerName);
...
bool HM_SdlSetupVideo() {
  ...
  hm_sdl.renderer =
      SDL_CreateRenderer(hm_sdl.window, -1, SDL_RENDERER_SOFTWARE);
