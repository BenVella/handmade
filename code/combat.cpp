...
inline const Hit create_hit(const Fighter &source, const Fighter &target,
...
inline Exchange *create_exchange(int size) {
  return new Exchange{size, create_fighter_array(size), nullptr};
}
...
// Ues when attacker <=15 nat, defender < 10 and overall mod roll is 5 or
// greater in favor of attacker
bool is_rolled_crit(const Hit &attacker, const Hit &defender, int attacker_mod,
                    int defender_mod) {
...
// Source indices are implicitly assigned starting from 1 unless explicitly
// provided. Target indices must be specified for 3 or more fighters. Modifier
...
int parse_rolls(const char *input, ParsedRoll *out, int max_rolls) {
  ...
  while (*p && count < max_rolls) {
    ...
    if (colon) {
      *colon = '\0';
      roll.src = strtol(cursor, NULL, 10);
      cursor = colon + 1;
    }
      ...
      *plus = '\0';
      ...
      if (dash) {
        *dash = '\0';
        ...
        roll.tar = strtol(dash + 1, NULL, 10);
      } else {
        roll.mod = strtol(cursor, NULL, 10);
